...
import com.lys.mobile.data.CabeceraInspeccionData;
import com.lys.mobile.data.MaquinasData;
import com.lys.mobile.util.DataBase;
import com.lys.mobile.util.Util;

public class CentroCostoAsyncTask extends AsyncTask<String, Integer, String> {
	...
	public CentroCostoAsyncTask(Context c) {
		super();
		context = c;
		...
		progressDialog.setMessage("Obteniendo Centros..");
			...
			try {
				...
				if (status.equals("0")) {
					...
					try {
						...
						int tamaño = Integer.parseInt(respJSON2
								.getString("size"));
						Gson gson = new Gson();
						...
						db.execSQL("DELETE FROM MTP_CENTROCOSTO");
	...
	protected void onPostExecute(String result) {// paso 4
		...
		if (result.equals("")) {
			...
			if (result.equals("0")) {
				...
				// boton.setEnabled(true);
				Toast t = Toast.makeText(context, "Usuario no existente!",
						Toast.LENGTH_SHORT);
				t.show();
				...
				final FamiliaAsyncTask tarea = new FamiliaAsyncTask(context);
				tarea.execute(new String[] { Util.url + "getFamilia" });
	...
	public void agregarbd(Object[] o, SQLiteDatabase db) {
		 ...
		 * 
		 * String fechaa2=String.valueOf(o[7]);
		 * 
		 ...
		 * 
		 * Date daten=null,datenu=null; SimpleDateFormat format = new
		 * SimpleDateFormat("MMM dd yyyy HH:mm:ss",Locale.ENGLISH); try { daten
		 * = (Date)format.parse(fechaaa); datenu = (Date)format.parse(fechaaa2);
		 * Log.d("date1", String.valueOf(daten)); //System.out.println(date); }
		 * catch (Exception e) { e.printStackTrace(); } SimpleDateFormat
		 * otroFormato = new
		 * SimpleDateFormat("yyyy-MM-dd HH:mm:ss",Locale.ENGLISH);
		 * System.out.println("FEcha : \n" + otroFormato.format(daten));
		 */

		db.execSQL("INSERT INTO MTP_CENTROCOSTO(c_compania,c_centrocosto,c_descripcion,c_estado) "
				+ "VALUES ('"
				+ o[0]
				+ "', '"
				+ o[1]
